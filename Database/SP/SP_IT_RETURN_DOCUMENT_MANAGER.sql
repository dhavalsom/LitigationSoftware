...
/****** Object:  StoredProcedure [dbo].[SP_IT_RETURN_DOCUMENT_MANAGER]    Script Date: 2/24/2019 9:04:28 AM ******/
...
DECLARE @Id AS BIGINT, @ITReturnDetailsId AS BIGINT, @ITHeadId AS BIGINT, @DocumentCategoryId AS BIGINT, @SubDocumentCategoryId AS BIGINT
...
SELECT	 @Id = ITReturnDocumentList.Columns.value('Id[1]', 'BIGINT')
	   ...
	   , @DocumentCategoryId = ITReturnDocumentList.Columns.value('DocumentCategoryId[1]', 'BIGINT')
	   , @SubDocumentCategoryId = ITReturnDocumentList.Columns.value('SubDocumentCategoryId[1]', 'BIGINT')
